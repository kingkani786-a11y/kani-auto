  ...
  return (
    <div className="panel border-terminal-warn/40 py-2.5">
      <div className="flex items-center justify-between flex-wrap gap-x-4 gap-y-1 text-sm">
        ...
        <span><span className="stat-label">Profit Lost</span> <span className="font-bold text-terminal-bear">+{m.potential_lost_pts ?? 0} pts</span></span>
        <span><span className="stat-label">Largest</span> <span className="font-mono">+{m.max_missed_pts}</span></span>
        <span><span className="stat-label">Top Blocker</span> <span className="text-terminal-bear">{m.worst_blocker ?? "—"}</span></span>
